...
*N.B.* Il programma **deve** immagazzinare i numeri inseriti da standard input all'interno di variabili di tipo `string`.
...
* `NuovaFrazione(numeratore, denominatore int) Frazione` che restituisce una istanza del tipo `Frazione` inizializzata in base ai valori dei parametri `numeratore` e `denominatore`;
